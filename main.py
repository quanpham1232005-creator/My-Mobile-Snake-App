...
async def main():
    try:
        ...
        # pygame.SCALED cho phép game tự động mượt mà giãn cách theo màn hình điện thoại
        ...
        font_path = os.path.join(s_path, "arial.ttf")
        if os.path.exists(font_path):
            font_sm = pygame.font.Font(font_path, 13)
            font_md = pygame.font.Font(font_path, 28)
            btn_font = pygame.font.Font(font_path, 18)
            go_font = pygame.font.Font(font_path, 55)
        else:
            font_sm = pygame.font.SysFont(None, 13, bold=True)
            font_md = pygame.font.SysFont(None, 28, bold=True)
            btn_font = pygame.font.SysFont(None, 18, bold=True)
            go_font = pygame.font.SysFont(None, 55, bold=True, italic=True)
